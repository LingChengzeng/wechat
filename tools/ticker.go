...
func (c *Ticker) Start(duration time.Duration) {
	...
	if c.ticker != nil {
		panic("Hamster Ticker: Cannot re-start a cache timer, if you stop it, it is not recommented to resume it,\n Just create a new CacheTimer.")
	}

	if duration.Seconds() <= 30 {
		//c.duration = 5 * time.Minute
		panic("Hamster Ticker: Please provide a duration that it's longer than 30 seconds.")
	}
	...
	go func() {
		for t := range c.ticker.C {
			...
			//			c.mu.Lock()
			//			c.mu.Unlock()
			// I can make it a clojure to handle only handlers that are
			// registed before .start() but we are ok with this, it is not
			// map no need to Lock, for now.
